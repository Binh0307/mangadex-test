...
export class ChapterComponent {
    ...
    private async findChapter(chapterNum: number, lang: string): Promise<Locator> {
        const hasPagination = await this.page.locator('button svg.feather-arrow-right').count() > 0;
        ...
        do {
            ...
            const nextEnabled = await this.isNextPageEnabled();
        ...
        throw new Error(`Chapter number not found after searching all pages. Remaining: ${chapterNum}, Language: ${lang}`);
    }
    ...
    private async isNextPageEnabled(): Promise<boolean> {
        const nextBtn = this.page.locator('button', {
            has: this.page.locator('svg.feather-arrow-right')
        });
